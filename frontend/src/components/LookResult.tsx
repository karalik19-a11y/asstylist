  ...
  return (
    <div className="stack page-transition result-page" style={{ gap: 18 }}>
      <section className="card stack result-hero" style={{ gap: 16 }}>
        <div className="row-between" style={{ gap: 10, alignItems: 'flex-start' }}>
          <div>
            ...
            <div className="result-title">{look.plan || 'Подборка'}</div>
          </div>
        ...
        <div className="unified-score">
          ...
          <div className="unified-score-main">
            ...
            <div className="small muted" style={{ marginTop: 4 }}>
              {fitHint ? `${fitHint} · ` : ''}
              {itemsWord(look.items.length)} · {formatRub(total)}
            </div>
            {thesisRu ? (
              <p className="result-thesis" style={{ margin: '10px 0 0', fontSize: 15.5, lineHeight: 1.5 }}>
                {thesisRu}
              </p>
            ) : null}
          </div>
        ...
        {look.summary ? (
          <p className="result-summary" style={{ margin: 0, fontSize: 16, lineHeight: 1.55 }}>
            {look.summary}
          </p>
        ) : null}

        <div className="result-actions">
          <button type="button" className="btn btn-primary btn-block" onClick={() => { playClick(); onFavorite() }}>
            <StarIcon size={16} /> {look.is_favorite ? 'В избранном' : 'В избранное'}
          </button>
          <button type="button" className="btn btn-outline btn-block" onClick={() => { playClick(); onNewLook() }}>
            Новый образ
          </button>
        </div>
      </section>
      ...
      <section className="card stack" style={{ gap: 12 }}>
        ...
        >
          <span>{detailsOpen ? 'Скрыть подробности' : 'Подробнее об образе'}</span>
          <span className="details-chevron" data-open={detailsOpen}>▾</span>
        </button>
          ...
          <div className="stack details-panel" style={{ gap: 16 }}>
            ...
            {look.body ? (
              <div className="stack" style={{ gap: 8 }}>
                <SectionTitle index="01">Фигура и посадка</SectionTitle>
            ...
            {look.palette ? (
              <div className="stack" style={{ gap: 8 }}>
                <SectionTitle index="02" hint={`точность ${Math.round((look.palette.confidence ?? 0) * 100)}%`}>
                ...
                {look.palette.source !== 'defaults' ? (
                  <div className="appearance-row">
                    {look.palette.skin_hex ? (
                      <span className="appearance-swatch" style={{ background: look.palette.skin_hex }} title="Тон кожи" />
                    ) : null}
                    {look.palette.hair_hex ? (
                      <span className="appearance-swatch" style={{ background: look.palette.hair_hex }} title="Волосы" />
                    ) : null}
                ...
                {activeColorInfo ? <div className="small muted">{activeColorInfo}</div> : null}
              </div>
            ) : null}

            {tasteMix.length ? (
              <div className="stack" style={{ gap: 6 }}>
                <SectionTitle index="03">Вкусовой микс</SectionTitle>
                <div className="wrap" style={{ gap: 8 }}>
                  {tasteMix.map(([key, value]) => (
                    <Badge key={key}>
            ...
            >
              {showDiagnostics ? 'Скрыть диагностику' : 'Диагностика подбора'}
            </button>
            {showDiagnostics ? (
              <ul className="reasons small">
                {diagnostics.plan_description ? <li>{diagnostics.plan_description}</li> : null}
                {diagnostics.candidates_total != null ? (
                  <li>Кандидатов: {diagnostics.candidates_total}</li>
                ) : null}
                {diagnostics.rejected_total != null ? <li>Отклонено: {diagnostics.rejected_total}</li> : null}
