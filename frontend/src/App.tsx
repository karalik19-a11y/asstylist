...
const SCREEN_TITLES: Record<Screen, string> = { home: 'ASStylist', wizard: 'Новый гардероб', result: 'Персональная селекция', history: 'Архив образов', profile: 'Личный кабинет', search: 'Поиск вещей', journal: 'Журнал' }
...
export default function App() {
  ...
  const refreshHistory = useCallback(async (favoriteOnly = false) => { try { const response = await api.history(100, favoriteOnly); setHistory(response.items) } catch { setHistory([]) } }, [])
  ...
  useEffect(() => { initTelegram(); setUserName(telegramUserName()); api.meta().then(setMeta).catch(() => setMeta(null)); api.auth().then((response) => setUserName(response.user.first_name ?? response.user.telegram_id)).catch(() => undefined); void loadBodyMemory().then(setSavedBody); void refreshHistory(false) }, [refreshHistory])
  const goHome = useCallback(() => { setScreen('home'); setError(null); void refreshHistory(false) }, [refreshHistory])
  ...
  const handleFavorite = useCallback(async () => { if (!look?.id) return; try { const response = await api.toggleFavorite(look.id); setLook({ ...look, is_favorite: response.is_favorite }); haptic('light'); void refreshHistory(historyFavoriteOnly) } catch (caught) { setError(errorMessage(caught)) } }, [look, historyFavoriteOnly, refreshHistory])
  ...
  const openHistory = useCallback(() => { setScreen('history'); setLoadingList(true); void refreshHistory(historyFavoriteOnly).finally(() => setLoadingList(false)) }, [refreshHistory, historyFavoriteOnly])
  const toggleHistoryFavorite = useCallback(() => { setHistoryFavoriteOnly((value) => { const next = !value; setLoadingList(true); void refreshHistory(next).finally(() => setLoadingList(false)); return next }) }, [refreshHistory])
  ...
  return (
    <div className="stage">
      ...
      <div className={`app${introDone ? ' app-ready' : ' app-booting'}`} data-screen={screen}>
        ...
        {screen === 'home' ? <Home meta={meta} userName={userName} theme={theme} onToggleTheme={toggleTheme} onStart={startWizard} onOpenSearch={openSearch} /> : null}
        ...
        {screen === 'history' ? <History items={history} loading={loadingList} favoriteOnly={historyFavoriteOnly} onToggleFavoriteOnly={toggleHistoryFavorite} onOpen={(id) => void openLook(id)} /> : null}
