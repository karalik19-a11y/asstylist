...
export default function App() {
  ...
  const refreshHistory = useCallback(async (favoriteOnly = historyFavoriteOnly) => { try { const response = await api.history(100, favoriteOnly); setHistory(response.items) } catch { setHistory([]) } }, [historyFavoriteOnly])
  ...
  const handleFavorite = useCallback(async () => { if (!look?.id) return; try { const response = await api.toggleFavorite(look.id); setLook({ ...look, is_favorite: response.is_favorite }); haptic('light'); void refreshHistory() } catch (caught) { setError(errorMessage(caught)) } }, [look, refreshHistory])
