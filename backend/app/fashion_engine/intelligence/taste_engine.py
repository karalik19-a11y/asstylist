"""TasteEngine — порт ``src/intelligence/TasteEngine.js``.

Считает Fashion Score (0…100), Taste Category и Anti-generic Score.
Исправление против оригинала: ``_assign_category`` передавал в
``_is_mass_market`` весь текст вместо бренда, из-за чего фильтр масс-маркета
не срабатывал никогда. Здесь передаётся бренд (см. README пакета).
"""
...
class TasteEngine:
    def evaluate(self, item: ProductItem, profile: UserStyleProfile | dict | None = None) -> TasteResult:
        ...
        text = f"{item.name or ''} {item.brand or ''}".lower().replace("ё", "е")
        ...
        fashion_score = round(
            uniqueness * 0.18
            + design_quality * 0.16
            + silhouette_strength * 0.14
            + min(100.0, cultural_relevance) * 0.10
            + editorial_value * 0.12
            + styling_potential * 0.12
            + niche_factor * 0.12
    ...
    def should_reject(self, item: ProductItem, taste: TasteResult, threshold: float = 55) -> bool:
        """Anti-generic фильтр: масс-маркет без вкуса в образ не попадает."""
        return taste.generic_score > 70 and taste.fashion_score < threshold
    ...
    def _uniqueness(self, item: ProductItem, attributes, text: str) -> float:
        ...
        if float(getattr(attributes, "rarity", 0) or 0) > 70:
            score += 30
        if any(word in text for word in ("archive", "rare", "limited", "архив", "лимит")):
            score += 25
        if getattr(attributes, "construction", "") == "deconstructed":
            score += 15
        if item.source_type in ("independent", "archive"):
            score += 10
    ...
    def _design_quality(self, attributes, text: str) -> float:
        ...
        if {"structured", "architectural"} & set(silhouette):
            score += 15
        if any(word in text for word in ("designer", "atelier", "ателье")):
            score += 10
        return min(100.0, score)
    ...
    def _niche_factor(self, item: ProductItem, attributes, text: str, profile: UserStyleProfile) -> float:
        ...
        if float(getattr(attributes, "rarity", 0) or 0) > 60:
            score += 25
        if any(word in text for word in ("independent", "emerging", "niche", "нишев")):
            score += 20
    ...
    def _generic_factor(self, item: ProductItem, attributes, text: str) -> float:
        ...
        if self._is_mass_market(item.brand or ""):
            score += 50
        if any(word in text for word in ("basic", "classic fit", "regular fit", "базов", "классическ")):
            score += 20
    ...
    def _assign_category(
        ...
        if score >= 90 and (float(getattr(attributes, "rarity", 0) or 0) > 80 or "cult" in text):
            return "exceptional"
        if score >= 85 and ("archive" in text or getattr(attributes, "historical_reference", None)):
            return "archive"
