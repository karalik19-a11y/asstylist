...
def _as_color_list(raw: Any) -> list[str]:
    ...
    if isinstance(raw, list): return [str(v) for v in raw]
    text = str(raw).strip()
    if not text: return []
    if text.startswith("["):
        try:
...
async def _read_photo(form: Any) -> tuple[bytes | None, str]:
    ...
    if upload is None or isinstance(upload, str) or not hasattr(upload, "read"): return None, ""
    if not getattr(upload, "filename", None): return None, ""
    ...
    if len(data) > settings.upload_max_bytes: raise HTTPException(status_code=413, detail="Файл слишком большой (максимум 10 МБ)")
    return data, content_type


def _resolve_user(session: Session, payload: dict[str, Any]):
    try: identity = authenticate(payload.get("init_data"), payload.get("demo_user_id") or payload.get("telegram_id"))
...
async def generate(request: Request, session: Session = Depends(get_session)) -> dict[str, Any]:
    ...
    vision: dict[str, Any] | None = None; photo_digest = ""; photo_path = ""
    if content_type.startswith("multipart/form-data"):
        form = await request.form(); payload = _form_to_payload(form); data, _ctype = await _read_photo(form)
        if data:
            try: vision = vision_ai.analyze(data)
            except VisionError as exc: raise HTTPException(status_code=422, detail=str(exc)) from exc
            ...
            if settings.store_photos:
                ...
                with open(photo_path, "wb") as handle: handle.write(data)
    else:
        try: raw = await request.json()
        except json.JSONDecodeError as exc: raise HTTPException(status_code=400, detail="Некорректный JSON") from exc
        if not isinstance(raw, dict): raise HTTPException(status_code=400, detail="Ожидался JSON-объект")
        payload = dict(raw)
    try: validated = LookRequest(**payload).model_dump()
    except Exception as exc: raise HTTPException(status_code=422, detail=f"Некорректные параметры: {exc}") from exc
    ...
    except LookGenerationError as exc: raise HTTPException(status_code=422, detail=str(exc)) from exc
    profile_service.remember_body(session, user, validated.get("height_cm"), validated.get("weight_kg"))
    payload_out = look_service.serialize_look(look); payload_out["profile"] = profile_service.get_profile(session, user)
    return payload_out
...
def history(request: Request, session: Session = Depends(get_session), limit: int = 100, favorite: bool = False) -> dict[str, Any]:
    ...
    items = look_service.list_looks(session, user.id, limit=min(max(limit, 1), 100))
    if favorite: items = [item for item in items if item.get("is_favorite")]
    return {"items": items}


@router.get("/{look_id}")
def get_one(look_id: int, request: Request, session: Session = Depends(get_session)) -> dict[str, Any]:
    user = _resolve_user(session, _query_payload(request)); look = look_service.get_look(session, look_id, user.id)
    if look is None: raise HTTPException(status_code=404, detail="Образ не найден")
...
def swap(look_id: int, request_body: dict[str, Any], request: Request, session: Session = Depends(get_session)):
    ...
    try: updated = look_service.swap_slot(session, look, slot, request_body.get("exclude_skus") or [])
    except LookGenerationError as exc: raise HTTPException(status_code=422, detail=str(exc)) from exc
...
def favorite(look_id: int, request: Request, session: Session = Depends(get_session)) -> dict[str, Any]:
    user = _resolve_user(session, _query_payload(request)); look = look_service.get_look(session, look_id, user.id)
    if look is None: raise HTTPException(status_code=404, detail="Образ не найден")
    look.is_favorite = not look.is_favorite; session.commit()
